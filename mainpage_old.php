	...
	/*		
		...
		else
		{
			*/
				if($display_name=='Administrator')
				{
						if($db_search->query("select  i.*,l.display_name from login l, iticket_info i  where Callername like '%$search%' && status='p' && l.user_id=i.user_id  or Companyname like '%$search%' && status='p' && l.user_id=i.user_id order by received_date DESC $limit_main ") && $db_search->rowCount() > 0)
						{
		...
		else if($_REQUEST['rad'] == 'c')
		{
			if($display_name=='Administrator')
			{
					if($db_search->query("select  i.*,l.display_name from login l, iticket_info i  where Callername like '%$search%' && status='c' && l.user_id=i.user_id or Companyname like '%$search%' && status='c' && l.user_id=i.user_id  order by received_date DESC $limit_main ") && $db_search->rowCount() > 0)
					{
		...
		else
		{
			if($display_name=='Administrator')
			{
					if($db_search->query("select  i.*,l.display_name from login l, iticket_info i   where Callername like '%$search%' and l.user_id =i.user_id  or Companyname like '%$search%' and l.user_id =i.user_id  order by received_date DESC $limit_main ") && $db_search->rowCount() > 0)
					{
...
else
{
	...
	if($_REQUEST['rad']=='c' || $status=='c')
	{
		if($display_name=='Administrator')
		{
			if($db_main->query("select  i.*,l.display_name from login l, iticket_info i where l.user_id = i.user_id and status='c' order by received_date DESC $limit_main ") && $db_main->rowCount() > 0)
			{
		 		// echo "Inside"; exit;
	...
	else if($_REQUEST['rad']=='p' || $status=='p')
	{
		...
		if($display_name=='Administrator')
		{
			if($db_main->query("select  i.*,l.display_name from login l, iticket_info i where l.user_id =i.user_id and status='p' order by received_date DESC $limit_main ") && $db_main->rowCount() > 0)
			{
	...
	else
	{
		if($display_name=='Administrator')
		{
			...
			//echo "select  i.*,l.display_name from login l, iticket_info i where l.user_id =i.user_id order by received_date DESC $limit_main" ;
			
			if($db_main->query("select  i.*,l.display_name from login l, iticket_info i where l.user_id =i.user_id order by received_date DESC $limit_main ") && $db_main->rowCount() > 0)
			{
