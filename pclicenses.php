<?php
	...
	// **************** FOR PAGING ****************
		...
		{
			$db_main->query("select * from userinfo u, pclicenses p where u.Assetid = p.Assetid ");
...
if($_REQUEST['src'])
{
		...
		//echo "select u.Assetid,u.UserName,u.CompanyName,u.Dept,u.Email,p.osname,p.oskey,p.officename,p.officekey,p.ipaddress,p.CPU_type,p.HardDrive,p.ram,p.previousos from userinfo u, pclicenses p where u.Assetid=p.Assetid and u.Username like '%$search%' or u.CompanyName like '%$search%' order by u.Assetid DESC";exit;
			...
			if($db_search->query("select u.Assetid,u.UserName,u.CompanyName,u.Dept,u.Email,p.osname,p.oskey,p.officename,p.officekey,p.ipaddress,p.CPU_type,p.HardDrive,p.ram,p.previousos from userinfo u, pclicenses p where u.Assetid=p.Assetid and u.Username ='$search' or u.Assetid=p.Assetid and u.CompanyName ='$search' order by u.CompanyName ASC $limit_main ") && $db_search->rowCount() > 0)
			{
...
else
{
	...
	//echo "select u.Assetid,u.CompanyName,u.Dept,u.Email,p.CPU_type,p.HardDrive,p.ipaddress,p.ram,p.osname,p.previousos,p.oskey,p.officename,p.officekey from userinfo u, pclicenses p where u.Assetid=p.Assetid order by u.Assetid DESC $limit_main ";exit;
			...
			if($db_search->query("select u.Assetid,u.UserName,u.CompanyName,u.Dept,u.Email,p.osname,p.oskey,p.officename,p.officekey,p.ipaddress,p.CPU_type,p.HardDrive,p.ram,p.previousos from userinfo u, pclicenses p where u.Assetid=p.Assetid order by u.CompanyName ASC $limit_main ") && $db_search->rowCount() > 0)
			{
	...
	$smarty->assign("pclicenses", $pclicenses);
...
/*	
	...
	$smarty->display("pclicenses.html");
