<?php
	...
	$db		=	new Database;
	 ...
	 $queryPie = "select IF(status='c', 'Completed', 'Pending') As Status, count(*) as Total from iticket_info where received_date between '$from' and '$to' group by status";  
	...
	$queryBarAgent = "SELECT SUM( i.status = 'c' ) AS Completed, SUM( i.status = 'p' ) AS Pending, l.display_name AS Agent
					FROM login l, iticket_info i
						WHERE i.user_id = l.user_id AND received_date between '$from' and '$to'
							GROUP BY Agent ";
	...
	$queryBarCompany = "SELECT SUM( status = 'c' ) AS Completed, SUM( status = 'p' ) AS Pending, companyname
					FROM iticket_info where received_date between '$from' and '$to' GROUP BY companyname ";
	$resultBarCompany = mysqli_query($connect, $queryBarCompany);
	
	$queryBarpbcat = "select iticket_cat, count(*) as Number from iticket_info where received_date between '$from' and '$to' group by iticket_cat";
